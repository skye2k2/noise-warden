...
- Pi 3 or better device and power supply with a minimum of:
  ...
  - 1GB available RAM (streaming finalization keeps memory usage under 100 MB even for 2-hour recordings; the 1024 MB systemd `MemoryMax` is the hard backstop)
...
| `audio` | Sample rate, block size, device selection, mic calibration offset, secondary/reference mic toggles, snippet pre/post buffer durations, min/max incident duration (`min_incident_seconds`, `max_incident_record_hours`), spectral denoising (enable + percentile/alpha/beta tuning), snippet normalization (enable + target peak dBFS) |
| `detection` | Zone, mode, armed state, calibration offset, noise floor gate, borderline margin + `record_borderline_events` toggle, day/night hours, music-focus thresholds, impulse/filter detection parameters, holdover tuning, drive-by auto-dismiss (including `driveby_sticky_threshold_sec`) |
...
- **Intelligent gating, recording, and logging of incidents** — hysteresis-based state machine prevents chatter at threshold boundaries; song-gap merging (default 10 sec) bundles closely-spaced noise events into single incidents. Incidents shorter than `min_incident_seconds` (default 20s) are auto-dismissed as transients. Long incidents are split at `max_incident_record_hours` (default 2h) to limit WAV file size and prevent memory issues during finalization. Optional `record_borderline_events: false` auto-dismisses incidents whose peak is within `borderline_margin_db` of the threshold (classified as 'borderline', snippet quarantined), keeping the log focused on clear violations. In `continuous_music_focus` mode, short incidents reclassified as non-music by the finalization keystone are auto-dismissed, while music-classified incidents are protected regardless of duration
- **WAV snippet evidence capture** — ring buffer preserves pre-trigger audio (`snippet_pre_seconds`, default 2s) plus post-trigger recording, saved per incident. A preroll warmup guard after engine startup ensures the first incident has proper lead-in context. Trimmed WAV endings receive a cosine fade-out (50 ms) to prevent DC-offset pops. Streaming finalization processes WAV files block-by-block without loading the entire file into memory, keeping peak usage under 100 KB even for multi-hour recordings
...
- **Reclassify tool** — replay the DSP pipeline against saved snippets to test config changes before deploying them. Available as both a CLI module (see [Reclassify CLI](#reclassify-cli) below) and an in-popup UI button on every incident with a stored snippet. Batch mode (`--all`) compares both classification and journal timeline, reporting change types. Also supports `--denoise` to batch-denoise ambient hiss (skips files >100 MB to preserve memory safety) and `--normalize` to batch-normalize snippet volumes after analysis
