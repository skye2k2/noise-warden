...
> **Why behind the what:** This file records what changed and when. For the
> reasoning behind significant and counter-intuitive decisions — why beat
> detection was removed, why the UI is infraction-first, the in-house-mic vs.
> studio-recording problem, the single-process architecture, the memory-limit
...
Incident management and portability improvements, and the start of an infraction-first (versus-classification-first) pivot.

<details>

- **Portable snippet path resolution** — the DB stores absolute snippet paths from the recording machine (`/opt/noise-warden/shared/snippets/...`). A database copied to a dev machine (using `./local_data/snippets/...`) could no longer play or reclassify its snippets. New `resolve_snippet_path()` in config.py resolves a stored path by **basename** against the current snippets directory (and its `autodismissed/` subfolder), falling back through: stored path → `{snippets_dir}/{basename}` → quarantine. Wired into the `/snippets/{id}` play route and both reclassify paths. The Pi's deployed behavior is unchanged (the stored path exists there). 5 new tests.
- **Fix orphan-purge wiping copied databases (regression)** — `purge_orphaned_incidents()` checked the raw stored path with `os.path.exists()`, so running it (or starting the engine, which runs it at startup) against a database copied from the Pi NULLed `snippet_path` for **every** row, because the `/opt/...` paths don't exist locally. It now uses the portable resolver and only nulls rows whose WAV cannot be found anywhere. Added `repair_snippet_paths()` + `--repair-snippet-paths` CLI flag to reconstruct `snippet_path` from the incident id embedded in `incident_{id}_*.wav` filenames, recovering databases already damaged by the old behavior. Maintenance-only CLI flags (`--purge-orphans`, `--repair-snippet-paths`) may now run standalone without a target. 6 new tests.
- **Remove beat confidence entirely** — `beat_confidence` (and the unreleased `beat_confidence_v2`) deleted from the DSP, engine, reclassify, storage, UI, and config. Validated against three months of hand-labeled real recordings: beat does **not** separate music from the dominant false positives, because aircraft, diesels, and helicopters are rhythmic machines whose blade-pass/firing periodicity (per-block beat 0.87–0.98) overlaps musical tempo entirely, while through-wall music loses its rhythm at the mic. No aggregation strategy rescues a non-separating feature. Consequences: `_classify_sound` and `analyze_clip` collapse the `music`/`music_like` split to a single tentative **`music_like`** label (the confident `music` label required a beat gate that no longer exists; spectral content alone cannot earn it); the `flyover_max_beat_confidence` and `amplified_bass_min_beat_confidence` filter gates and the `beat_confidence` plumbing through `identify_filter` and all `_check_*` dispatchers are gone; the `beat_confidence` DB column is dropped via schema migration v4. README gains a "why we no longer track beat confidence" caveat. Beat test classes removed; 559 tests pass.
- **Finalize classification keystone (engine ↔ reclassify parity)** — `_finalize_incident()` now derives the stored classification, journal, and `music_like_score` by running the **same `analyze_clip()` path the reclassify tool uses**, on the final (trimmed → denoised → normalized) WAV. Previously the engine stored the *first block's* `music_like_score` snapshot — taken before the spectrum was representative — which is why confirmed incidents could show a misleading score. Now re-running `reclassify` after any engine change reproduces the engine's own numbers exactly (parity by construction). When recording is disabled (no WAV), it falls back to the live journal via the shared `_compute_dominant` helper, leaving `music_like_score` untouched. The inline journal-duplication block in the engine was deleted in favor of this single source of truth. 1 new keystone integration test.
- **Body-windowed avg_db** — `avg_db` is now averaged over the incident *body* (trigger → last above-threshold block) instead of over the trimmed dbs that still included the retained `snippet_post_seconds` context tail. A multi-second sub-threshold tail was dragging the average down and understating real loudness. Loudness stays computed on the **raw** dbs (never the normalized WAV), since normalization rewrites peak amplitude and would corrupt calibrated dBA; `peak_db` is unchanged (already body-dominated).
- **Body-median music_like_score** — `analyze_clip()` now returns `music_like_median`, the median `music_like_score` across body blocks only (lead-in / lead-out excluded). Median resists transient single-block spikes; excluding bookends prevents a quiet preroll or fade-out from skewing the value. Both the finalize keystone and reclassify `--update` store this instead of a single block's value.
- **Tightened engine midband veto (0.50 → 0.40)** — `_classify_sound` (and the matching `analyze_clip` classifier) now reclassify a block as `engine_noise` when `midband_ratio` exceeds the new configurable `engine_midband_veto` (default 0.40, was a hardcoded 0.50). Calibrated from labeled data: confirmed-music midband tops out near 0.22 (p75) while flyover/diesel/helicopter routinely exceed 0.40, so the tighter veto removes ~9× more engine false-positives (9/40 vs 1/40 sampled) while vetoing **zero** confirmed-music incidents — a strict improvement, not a trade. 3 new tests (boundary + configurability).
- **Infraction-first incidents table** — the Class column is now hidden by default behind a "Show diagnostics" toggle (preference persisted in localStorage). Classification is a tentative hint, not enforcement evidence, so the table leads with loudness/duration/excess and reveals the class only on opt-in. The detail popup still shows classification — that is the diagnostic view.
- **Mode-aware auto-dismiss in music focus** — in `continuous_music_focus` mode, the `drive_by` and `too_short` auto-dismiss checks are skipped. An incident only exists in that mode because a block classified as `music_like` (the run-loop mode gate), so it is by construction a music detection — exactly what we are hunting. Previously a brief music burst that dipped below threshold and rose again (e.g. the 2026-05-31 21:55 incident 8369, a 4s 0.89-mscore burst) was hidden as a drive-by. Borderline (loudness-margin) dismissal still applies in all modes. 3 new tests.
- **music_focus_gap_merge_sec** — new config key (default 45s) used only in `continuous_music_focus` mode for the gap-merge window before an active incident finalizes; other modes keep `song_gap_merge_sec` (12s). Music dips below threshold between songs / during quiet passages / when a vehicle briefly masks the bass, so a longer window keeps one nuisance session as a single incident rather than fragmenting it into several (and stops a brief re-rise from being logged separately). Falls back to `song_gap_merge_sec` if the key is absent. 3 new tests.
- **Per-classification filter on incidents page** — collapsible "Classification Controls" filter panel with checkboxes for each auto-dismissed (borderline, drive_by, too_short) and filter-excluded (amplified_bass, birdsong, conversation, diesel, impulse, mower, rain, thunder, weedwhacker) classification. Toggling a checkbox adds those excluded incidents to the normal view. Filter state persists across pagination via URL query params (`?show=drive_by,too_short`). Excluded rows are visually dimmed. 2 new tests
- **Fix auto-dismiss snippet_path** — the three auto-dismiss blocks (drive_by, too_short, borderline) now update `snippet_path` in the DB to the quarantine location after `shutil.move()`. Previously, `snippet_path` still pointed to the original (now nonexistent) path, making quarantined files unplayable and creating orphaned references. Quarantined incidents are now playable when viewing excluded classifications.
- **Fix \"lead-in\" leaking into primary classification** — both `_finalize_incident()` (engine) and `_compute_dominant()` (reclassify) excluded bookend classes (`lead-in`, `lead-out`) from the gating check but not from the `durations` fallback. When all non-bookend classes were ignorable (unknown, engine_noise), `lead-in` could win on raw duration — producing classifications like `lead-in+`. Bookends are now filtered from the fallback dict. 1 new regression test.
- **Hard-delete on UI deletion** — `soft_delete_incident()` now performs a hard DELETE (removes the DB row entirely) instead of setting `deleted=1`. This prevents orphaned ghost rows from accumulating and inflating counts in reclassify-all. The WAV file is still removed from disk (checking both original and quarantine paths) before the row is deleted.
- **Clear Auto-Dismissed button** — new `POST /incidents/clear-autodismissed` route and "Clear Auto-Dismissed Incidents" button. Hard-deletes all `excluded=1` DB rows and removes all WAV files from the `autodismissed/` quarantine directory. VACUUMs the DB to reclaim space. 2 new tests
- **Orphan cleanup at startup** — `purge_orphaned_incidents()` now runs automatically at engine startup (regardless of `auto_purge_enabled`). NULLs out `snippet_path` for any incident whose WAV no longer exists on disk. This is a data-integrity operation — prevents "832 skipped (no file)" inflation in reclassify-all after manual file cleanup.
- **Fix file ownership to avoid SSH issues transferring data** — Add a UMask to ensure that all users can read created files.

</details>

## v16 - 2026-05-22 — "Stability is King Edition"

Crash-repair hardening, UI cleanup, CPU temperature monitoring, and network status visibility.

<details>

- **Crash-repaired incident WAV re-attachment** — `repair_stale_incidents()` now accepts an optional `snippets_dir` parameter. When provided, it globs for `incident_{id}_*.wav` orphaned temp files and re-attaches them to `snippet_path` before marking the incident crash-repaired, so a partial recording is not lost. The engine passes the snippets directory at startup. Previously, crash-repaired incidents always appeared audio-less even when a partial WAV existed.
- **Bulk-delete crash-repaired incidents** — new `storage.delete_crash_repaired_incidents()` hard-deletes all incidents carrying the crash-repair sentinel in their notes (and removes any re-attached partial WAV). Exposed via `POST /incidents/clear-crash-repaired` and a "Clear Crash-Repaired Incidents" button on the incidents page. Addresses the accumulation of ~20 zero-duration ghost rows that could not previously be removed through the UI.
- **Per-incident delete via overlay only** — individual incident deletion is accessible through the play-button overlay's "Delete incident" button, keeping destructive actions behind a listen-first workflow. The separate per-row delete button was removed after determining it was redundant once calibration is dialed in. Incidents without audio (e.g., crash-repaired) are handled by the bulk "Clear Crash-Repaired" action.
- **Fresh disk space on dashboard page load** — the dashboard route now calls `shutil.disk_usage()` directly when rendering, overwriting the potentially 24-hour-stale engine state value. The disk pill now reflects actual current usage on every page load/reload, not just within 5 seconds of the next poll cycle.
- **CPU temperature monitoring** — `_check_cpu_temp()` reads `/sys/class/thermal/thermal_zone0/temp` at startup and every 30 seconds thereafter, regardless of armed state. Publishes `cpu_temp_c` and `cpu_temp_warning` to state. Dashboard shows a temperature pill: amber at ≥75°C (throttle threshold), red at ≥80°C (sustained throttling). Pill is created dynamically by the JS poll on first data, so dev environments without sysfs simply show nothing. 3 new tests
- **WiFi link status monitoring** — `_check_network()` reads `/sys/class/net/wlan0/operstate` every 60 seconds, regardless of armed state. Logs link-down/up transitions to journald for post-incident diagnosis. Publishes `network_ok` and `network_warning` to state. Dashboard shows a warning banner when the Pi's own WiFi link is down — distinct from the existing browser-connectivity gray-out, which fires when the server is unreachable from the browser side. Interface is configurable via `app.network_interface` (defaults to `wlan0`). Silently skips on macOS/container. 2 new tests
- **Config path fallback chain** — `config.py` now resolves config path via: `NOISE_WARDEN_CONFIG` env var → `/opt/noise-warden/current/config/noise_warden.yaml` (deployed Pi) → `config/noise_warden_local.yaml` (local dev). If none exist, raises `ConfigError` with an actionable message explaining how to create the local override. Previously, a missing config produced a raw `FileNotFoundError` with a confusing path.
- **Eliminated hardcoded config paths in web.py** — all 7 occurrences of `os.environ.get("NOISE_WARDEN_CONFIG", "/opt/noise-warden/...")` replaced with `_default_config_path()` from config.py. The `/config` page, config save, `_persist_armed()`, calibration, sample-rate, noise-floor, and detection-mode routes all now use the same fallback chain.
- **CSS pill modifier classes** — extracted inline `style=` attributes for active/warn/danger pill states into `.pill--active`, `.pill--warn`, `.pill--danger` CSS classes in style.css. Dashboard server-render and JS poll now use `classList.toggle()`.
- **Dashboard warning banner JS fix** — the poll loop's banner update section was missing its outer structure (variable declarations, conditional checks for paused/recording-disabled states). Reconstructed the complete block so banners update correctly on each poll cycle.
- **Throttle status monitoring** — `_check_throttle()` calls `vcgencmd get_throttled` every 30 seconds (alongside `_check_cpu_temp()`). Decodes the bitmask into human-readable conditions: under-voltage, freq-capped, throttled, soft-temp-limit. Publishes `cpu_status` to state. Dashboard shows a red "CPU Status" pill when any current condition is active; pill auto-removes when conditions clear. Silently skips on macOS/container where vcgencmd is unavailable. 4 new tests
- **testing_overrides config section** — new optional `testing_overrides` section with an `enabled` master switch, `cpu_temp_c`, and `cpu_status` keys. When enabled, `_check_cpu_temp()` and `_check_throttle()` use the override values instead of reading sysfs/vcgencmd, allowing full dashboard UI testing on non-Pi hardware. Config is loaded once at startup; in local dev, `--reload-include '*.yaml'` auto-restarts uvicorn on YAML changes. On the Pi (systemd, no `--reload`), a manual `systemctl restart` is needed. 2 new tests (override path + disabled-skips-override)
- **CPU temp check interval** — `_check_cpu_temp()` now fires every 30 seconds (was startup + daily). Runs before the armed gate alongside the network check so it updates even when paused.
